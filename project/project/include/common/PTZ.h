...
typedef enum protocol_support_action
{
	...
	PROTOCOL_DISABLE_CONFIGURE_CRUISE  = 0x0010,
	PROTOCOL_SUPPORT_AUXILIARY	= 0x0020,
}PROTOCOL_SUPPORT_ACTION;

const char PTZ_PROTOCL_NAME [][64] = 
{ 
	"NULL",
	"PELCOP", 
	"PELCOD", 
...
inline unsigned long PTZSupportMask( unsigned long ProtocolID)
{
	...
	case PROTOCOL_PELCOP:
		{
			mask = PROTOCOL_SUPPORT_CURISE | PROTOCOL_SUPPORT_TRACK | PROTOCOL_SUPPORT_PRESET | PROTOCOL_SUPPORT_AUTOSCAN | PROTOCOL_SUPPORT_AUXILIARY;
			break;
		}
	...
	case PROTOCOL_PL14:
		{
			mask = PROTOCOL_SUPPORT_CURISE | PROTOCOL_SUPPORT_TRACK | PROTOCOL_SUPPORT_PRESET | PROTOCOL_SUPPORT_AUTOSCAN | PROTOCOL_DISABLE_CONFIGURE_CRUISE;
			break;
		}
	case PROTOCOL_TVI:
		{
...
inline bool PTZSupportAction( unsigned long ProtocolID, PROTOCOL_SUPPORT_ACTION PTZAction )
{
	...
	else if( PROTOCOL_DISABLE_CONFIGURE_CRUISE == PTZAction)
	{
		switch( ProtocolID)
		{
		case PROTOCOL_PL14:
	...
	else if (PROTOCOL_SUPPORT_AUXILIARY == PTZAction)
	{
		switch( ProtocolID )
		{
			//支持自动扫描的协议
...
typedef enum _ptz_com_type
{
	...
	PTZ_CMD_TRACK_SET,			//开始保存轨迹
	PTZ_CMD_TRACK_DEL,			//停止保存轨迹

	PTZ_CMD_AUTO_SCAN_START,	//开始自动扫描
	PTZ_CMD_AUTO_SCAN_STOP,		//停止自动扫描

	PTZ_CMD_POP_PTZ_MENU,		//弹出云台自带菜单
	
	PTZ_CMD_SET_AUXILIARY,		//打开辅助开关
	PTZ_CMD_CLEAR_AUXILIARY,	//关闭辅助开关
...
//云台辅助开关
enum _ptz_auxiliary_
{
	AUXILIARY_START			= 0,
	AUXILIARY_BRUSH			= 1,	//雨刷控制
	AUXILIARY_LAMP			= 2,	//灯光
	AUXILIARY_VIDEO_POWER	= 3,	//摄像头电源开关
	AUXILIARY_END			= 9,
};
...
enum PTZ_STATE
{
	...
	PTZ_ST_BRUSH_ON = 0x0010,	//雨刷打开
	PTZ_ST_LAMP_ON = 0x0020,	//灯光打开
