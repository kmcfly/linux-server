...
#include "Typedef.h"
#if defined(__CHIP3520A__) || defined(__CHIP3535__) || defined(__HISI_SDK_0BX__) || defined(__CHIP3521__) || defined(__CHIP3531__) || defined(__CHIP3520D__)
#include "mylist.cpp"
#else
#include "mylist.h"
#endif
...
typedef enum _DDNS_SERVER_TYPE
{
	...
	DDNS_NOIPDNS,
	DDNS_DVRDYDNS,
	DDNS_MINTDNS,//使用DYNDNS的微型版本，DDNS_NEWDDNS、DDNS_EASTERNDNS、DDNS_DVRLIS、DDNS_MYQSEE都为这种类型。参考http://www.dyndnsservices.com获取更多信息
	...
	DDNS_MINCO,			//TW01专用
	DDNS_TECVOZ,		//BR27专用
	DDNS_WINCO_BR27,	//BR27 WINCO服务器
	DDNS_MINT_AUTO,		//同为自动注册DDNS
	DDNS_HIVIEW,
	DDNS_MX15,			//MINT ddns类型
	DDNS_VN11,			//MINT ddns类型
	DDNS_CNB,			//CNB 的DDNS
	DDNS_END,
}DDNS_SERVER_TYPE;
...
typedef enum _vga_resolution
{
	...
	VGA_1440X960	= 0x0100,
	VGA_1440X1152	= 0x0200,
	VGA_1280X720	= 0x0400,
	VGA_RESOLUTION_NONE = 0xffff,
}VGA_RESOLUTION;
...
const unsigned char MAX_VIDEO_CONFIG_ROI_NUM =8;//如果最大值变了，请修改对应的它DD_MAX_VIDEO_ROI_NUM = 8
typedef struct{
	VIDEO_CONFIG_ROI roi[MAX_VIDEO_CONFIG_ROI_NUM];
}VIDEO_CONFIG_ROI_INFO;
...
typedef enum _alarm_type
{
	...
	ALARM_TYPE_DISK_DISCONNET		=0x0081,
...
typedef enum _other_alarm_type
{
	...
	OTHER_ALARM_TYPE_DISK_DISCONNET,
	OTHER_ALARM_TYPE_NUM
}OTHER_ALARM_TYPE;

//异常报警级最高，不需要配置，可以自由控制蜂鸣器开关
typedef enum _exception_alarm_type
{
	EXCEPTION_ALARM_TYPE_POE_NET,	//POE网络异常,导致所有设备掉线 
}EXCETION_ALARM_TYPE;
...
typedef struct _encode_info_record__
{
	unsigned char	resolution;		//图像分辨率
	unsigned char	rate;			//帧率(未最初的值为 0（初始化个无意义的帧率值）)
	unsigned short	change;			//0（没有变化）1（升高）2（降低）
}ENCODE_INFO_RECORD;
...
typedef struct _encode_info_ex
{
	unsigned long	resolution;		//图像分辨率
	unsigned short	rate;			//编码帧率
	unsigned char	encodeType;		//编码类型：定码流和变码流
	unsigned char	quality;		//编码质量
	unsigned long	lBitStream;		//编码码率下限
	unsigned long	hBitStream;		//编码码率上限
...
typedef struct _file_info_ex_backup 
{
	FILE_INFO_EX  fileInfo;
	unsigned long hasBackupAudio;
}FILE_INFO_EX_BACKUP;
...
typedef enum _log_type
{
	...
	LOG_TYPE_IPCAM_ADD,							//添加设备
	LOG_TYPE_IPCAM_SETUP,						//调整设备
	LOG_TYPE_IPCAM_DELETE,					//删除设备
	LOG_TYPE_IPCAM_CONNECTED,			//设备连接上
	LOG_TYPE_IPCAM_DISCONNECTED,	//设备断开连接
...
typedef enum _language_
{
	...
	LANGUAGE_VIETNAMESE    = 0x0400000,
	LANGUAGE_PERSIAN       = 0x0800000,
	LANGUAGE_DUTCH         = 0x1000000,
	LANGUAGE_FINNISH       = 0x2000000,
	LANGUAGE_SWEDISH       = 0x4000000,	
	LANGUAGE_DANISH        = 0x8000000,
...
typedef struct _ddns_info_ex_
{
	...
	char	password[132];		//用户密码
	char	domain[132];		//对应的域名信息。
	char	serverDNS[132];		//服务器域名。mintdns类型服务器使用。
}DDNS_INFO_EX;
...
typedef struct _use_ipv4_ipv6
{
	unsigned long bEnableIPv4;	//是否启用IPv4
	unsigned long bEnableIPv6;	//是否启用IPv6
}USE_IPV4_IPV6;
...
typedef struct _network_addr_ipv6 
{
	unsigned short	recv;						//保留
	unsigned short	useDHCP;				//是否使用动态网络地址

	unsigned char IP[40];		
	unsigned long subnetMask;
	unsigned char gateway[40];
}NETWORK_ADDR_INFO_IPv6;

typedef struct _net_switch_info
{
	unsigned long	switchIP;
	unsigned long	netMask;
}NET_SWITCH_INFO;
...
typedef struct _wireless_status_info
{
	unsigned long	IP;				//无线网络地址
	unsigned long	wirelessstatus;	//无线网络连接地址 1为连接成功，0为连接失败
	unsigned long	reserved1;		//保留位
	unsigned long	reserved2;		
	unsigned long	reserved3;
	unsigned long	reserved4;
}WIRELESS_STATUS_INFO;
...
typedef struct _net_auto_report_
{
	unsigned long   bUse;
	char			host[256];
...
typedef struct mdvr_dialup_cfg
{
	char	ConnectPoint[68];			//拔号接入点
	char	APNNumber[68];				//APN业务号
	char	username[68];				//用户名
	char	password[36];				//密码
}MDVR_DIALUP_CFG;
...
{
	unsigned short	enable;			//是否开启无线网络功能 1:开启，0：关闭
	unsigned short 	type;			//无线模块类型1:WCDMA 2:EVDO
	unsigned short 	mode;			//连接模式1:长连模式2:激活模式
	unsigned short  reserved;		//预留
...
typedef struct __push_server_info__
{
	char szServerAddr[128];
	unsigned long iServerPort;
}PUSH_SERVER_INFO;
...
typedef enum _func_type
{
	...
	FUNC_BACKUP,					//远程备份
	FUNC_PLAYBACK,					//远程回放
	FUNC_AUTOREPORT,				//自动上报
	FUNC_WIRELESS,					//无线网络	
	FUNC_NO_ALARM_CTRL,				//是否支持报警输入输出。 0-- 支持 1--不支持
	FUNC_HAVE_IPV6,					//是否支持IPV6。0--不支持，1--支持
	FUNC_SUPPORT_FTP,				//是否支持FTP功能，0不支持，1支持
	FUNC_IS_MIXED_DVR,				//是否是混合型DVR，0表示不是， 1表示是
	FUNC_CAN_CONTROL_SUBSTREAM,		//是否可以控制子码流，0表示不是， 1表示是；主要给手机客户端用来控制NVR的子码流编码参数
	FUNC_USE_STATIC_VIDEO_BITSTREAM,//NVR和HYBIRD设备，用来控制每个分辨率是否使用静态码流大小
	FUNC_SERVER_ENCODE_CHECK,		//设备端控制编码参数配置
	FUNC_SUPPORT_PUSH,				//是否支持推送功能
	FUNC_SUPPORT_SET_IP_ENCODE,		//是否支持设置网络编码参数
	FUNC_SUPPORT_SET_IP_SUB_ENCODE,	//是否支持设置网络通道的子码流编码参数配置
	FUNC_SUPPORT_SET_IP_PTZ_CONFIG,	//是否支持网络通道的云台配置功能
	FUNC_SUPPORT_LOCAL_SUB_STREAM,  //是否支持设置子码流
}_FUNC_TYPE;
...
const unsigned int TVT_MAX_CAMERA_NUM		= 32;
const unsigned int TVT_MAX_LANGUAGE_NUM		= 64;
const unsigned int TVT_MAX_TITLE_LEN		= 64;
const unsigned int TVT_MAX_URL_LEN			= 256;
const unsigned int TVT_MAX_TEXT_LEN			= 256;
const unsigned int TVT_MAX_ACCOUNT_LEN		= 128;
const unsigned int TVT_MAX_PASSWORD_LEN		= 32;


typedef struct _tvt_ftp_config_
{
	unsigned short	enableFTP;			//是否启用FTP		
	unsigned short	uploadType;			//上传文件类型，ASSCII方式或者二进制的流方式。默认为二进制的流方式

	unsigned short	cameraNum;			//有效通道数目（只读）
	unsigned short	uploadPicNum;		//上传图片的张数（每个通道），零表示不上传
	unsigned short  uploadCamNum;       //机器支持上传视频的通道个数(只读)
	unsigned short	uploadVideoLen;		//上传视频的大小，单位为M，当前设置为32M~64~
	unsigned int	streamType;			//上传视频流类型（TVT_STREAM_TYPE）
	...
	unsigned short	serverPort;			//服务器端口（默认21）
	unsigned short	anonymous;			//是否匿名登录
	unsigned int	encryption;			//加密方式:ENCRYPTION_TYPE

	char	serverURL[TVT_MAX_URL_LEN+4];//服务器地址（可以是IP地址，也可以是域名）
	char	account[TVT_MAX_ACCOUNT_LEN+4];
	char	password[TVT_MAX_PASSWORD_LEN+4];
	char	remotePath[TVT_MAX_URL_LEN+4];
}TVT_FTP_CONFIG;

//zxx新加

typedef struct _Setup_datainfo_
{
	unsigned long cfgID;//需要刷新的ID
	//NetWork
	NETWORK_ADDR_INFO network_info;//NCFG_ITEM_NETWORK_ADDR_INFO
	unsigned long pUlong1;//NCFG_ITEM_DNS_1
	unsigned long pUlong2;//NCFG_ITEM_DNS_2

	//NCFG_ITEM_PPPOE_INFO
	unsigned long maxUserLen;
	unsigned long maxPassLen;
	PPPOE_INFO pPPPOE;

	//NCFG_ITEM_SWITCH_INFO
	unsigned long	switchIP;
	unsigned long	netMask;

	//WireLess
	MDVR_WIRELESS_CFG mdvr_wireless;
}SETUPDATAINFO;


//国标相关
#define GB28181_SHORT_NAME_LEN	32
#define GB28181_LONG_NAME_LEN	64

typedef struct _gb28181_sip_server
{
	unsigned long		server_ip;							//sip server IP 地址
	unsigned short		server_port;						//sip server 端口
	unsigned int		expires;							//过期时间
	unsigned char		server_id[GB28181_SHORT_NAME_LEN];	//服务器id
	unsigned char		user[GB28181_SHORT_NAME_LEN];		//注册用户名
	unsigned char		passwd[GB28181_SHORT_NAME_LEN];		//注册密码
	unsigned char		dvr_id[GB28181_SHORT_NAME_LEN];		//dvrid
	unsigned short		dvr_port;							//dvr 端口
	char				devowner[GB28181_LONG_NAME_LEN];	//设备归属
	char				civilcode[GB28181_LONG_NAME_LEN];	//行政区域
	char				installaddr[GB28181_LONG_NAME_LEN];	//安装地址
}GB28181_SIP_SERVER;

typedef struct _gb28181_channel_id
{
	char channel_id[TVT_MAX_CAMERA_NUM][GB28181_SHORT_NAME_LEN];
}GB28181_CHANNEL_ID;

typedef struct _gb28181_alarm_id
{
	char alarm_id[TVT_MAX_CAMERA_NUM][GB28181_SHORT_NAME_LEN];
}GB28181_ALARM_ID;
...
//网络设备支持的编码信息
typedef struct __net_channel_enc_info__
{
	unsigned long	chnn;			//通道
	unsigned long	online;			//是否在线, 0，不在线，1，在线
	struct __enc_info__
	{
		unsigned long	videoSize;		//分辨率 VIDEO_SIZE
		unsigned char	ucRate;			//每种分辨率对应的最大帧率, 比如 D1对应最大帧率: ucRate
		unsigned long	maxBitRate;		//每种分辨率对应的最大码率，比如 D1对应最大码率: maxBitRate
	}encInfo[32];
}NET_CHANNEL_ENC_INFO;

#endif

typedef struct __check_enc_info__
{
	unsigned long  ulRes;		//分辨率
	unsigned short usRate;		//帧率
	unsigned short usBitrate;	//码率
}CHECK_ENC_INFO;
...
typedef struct support_sub_enc_info__
{
	unsigned short  chnn;           //通道号
	unsigned long	ulRes;			//分辨率
	unsigned short	usMaxRate;		//最大帧率
	unsigned short	usMaxBitrate;	//最大码率
	unsigned long	ulDefBitrate;	//默认码率
}SUPPORT_SUB_ENC_INFO;

typedef struct _af_control__
{
	int          chnn;
	AF_CTRL_TYPE afType;
...
typedef enum __file_buf_type__
{
	FILE_BUF_TYPE_LOCAL,
	FILE_BUF_TYPE_IP,
	FILE_BUF_TYPE_MIN,
...
#if defined(__CHIP3520A__) || defined(__CHIP3535__) || defined(__HISI_SDK_0BX__) || defined(__CHIP3521__) || defined(__CHIP3531__)
