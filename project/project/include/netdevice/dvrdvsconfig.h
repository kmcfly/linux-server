...
const unsigned long DD_MAX_VIDEO_ROI_NUM = 8;//如果也修改对应的 MAX_VIDEO_CONFIG_ROI_NUM =8
...
typedef struct _dd_basic_config_
{
	...
	unsigned long videoOutResolution;	//输出分辨率
	...
	//以下为一些只读项
	unsigned long videoFormatMask;			//支持的视频制式掩码（只读）
	unsigned long videoOutMask;				//支持的视频输出设备掩码（只读）
	unsigned long videoOutResolutionMask;	//支持的视频输出设备分辨率掩码（只读）
...
typedef struct _dd_video_roi_config_
{
	ROI_CONFIG roi[DD_MAX_VIDEO_ROI_NUM];
}DD_VIDEO_ROI_CONFIG;

typedef struct _nfcg_lens_distortion_config_
{
	unsigned char  ucLensDistortion;        // 0 no, 1 yes
	unsigned char  ucLensDistortionValue;   // 0 - 255
	char            noUse[2];
}DD_LENS_DISTORTION_CONFIG;

typedef struct _dd_encode_config_
{
	unsigned long iSize;		//本结构体长度

	unsigned short resolution;	//分辨率
	unsigned short rate;		//帧率
	unsigned short encodeType;	//编码类型
	unsigned short quality;		//画质
...
typedef struct _dd_account_config_
{
	...
	unsigned long bingMAC;			//是否绑定物理地址
	unsigned long group;			//所属群组
...
{
	unsigned char toPTZType;	//联动类型（预置点、巡航线、轨迹）
...
typedef struct _dd_motion_area_
{
	...
	unsigned short hightNum;		//区域的纵向栅格数目
...
typedef struct _dd_ptz_protocol_info 
{
	...
	unsigned long  pportproperty;		//支持的除ID、波特率之外的其它属性的MASK,比如是否支持轨迹等特殊属性
...
typedef enum dd_config_item_id
{
	...
	DD_CONFIG_ITEM_VIDEO_ROI,               // DD_CONFIG_ITEM_VIDEO_ROI * NET_VIDEO_INPUT_NUM           
	DD_CONFIG_ITEM_CAM_MIRROR,			    //DD_CONFIG_ITEM_CAM_MIRROR * NET_VIDEO_INPUT_NUM
	DD_CONFIG_ITEM_CAM_FILP,				//DD_CONFIG_ITEM_CAM_FILP * NET_VIDEO_INPUT_NUM
	DD_CONFIG_ITEM_CAM_ROTATO,              //DD_CONFIG_ITEM_CAM_ROTATO * NET_VIDEO_INPUT_NUM
	DD_CONFIG_ITEM_CAM_LENS_DISTORTION,	    //DD_CONFIG_ITEM_CAM_LENS_DISTORTION * NET_VIDEO_INPUT_NUM
	DD_CONFIG_ITEM_CAM_TIME_FOCUS_MODE,	    //DD_CONFIG_ITEM_CAM_TIME_FOCUS_MODE * NET_VIDEO_INPUT_NUM
	DD_CONFIG_ITEM_CAM_IRCHANGE_FOCUS,	    //DD_CONFIG_ITEM_CAM_IRCHANGE_FOCUS * NET_VIDEO_INPUT_NUM
	DD_CONFIG_ITEM_CAM_INTERVAL_TIME,	    //DD_CONFIG_ITEM_CAM_INTERVAL_TIME * NET_VIDEO_INPUT_NUM
	...
	DD_CONFIG_ITEM_ENCODE_JPEG,				//DD_ENCODE_CONFIG * VIDEO_INPUT_NUM
	DD_CONFIG_ITEM_ENCODE_THIRD_STREAM,
	...
	DD_CONFIG_ITEM_PTZ_PROTOCOL_INFO = 0x0d03, //DD_PTZ_PROTOCOL_INFO
	DD_CONFIG_ITEM_PTZ_CRUISE = 0x0d04,
    DD_CONFIG_ITEM_PTZ_CRUISE_END = DD_CONFIG_ITEM_PTZ_CRUISE + 0x40,
