...
#ifndef _CFG_LIVE_COLOR_DLG_
#define _CFG_LIVE_COLOR_DLG_
...
class CCfgLiveColorDlg : public GUI::CDialog
{
public:
	CCfgLiveColorDlg();
	~CCfgLiveColorDlg();
...
public:
	...
	//max最大通道数量
	//sel = [0,max-1]
	void SetChnnInfo(const unsigned long*pColor,int max,int sel=-1);

	void SetChnnInfo(const unsigned long*pColor,int max,int sel,int flag);//重载函数，适应工具条和右键菜单修改

	void GetChnnColor(unsigned long* pColor,int len);
...
protected:// 变量
	...
	//LONGLONG			m_sendTime;						//发送色彩的时间（防止发送太频繁）
	int					m_localVideoNum;
};
