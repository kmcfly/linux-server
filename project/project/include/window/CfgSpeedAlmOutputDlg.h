/***********************************************************************
** Copyright (C) Tongwei Video Technology Co.,Ltd. All rights reserved.
** Author       : lufen
** Date         : 2010-03-31
** Name         : CfgSpeedAlmOutputDlg.h
** Version      : 1.0
** Description  : 
** Modify Record:
1:
***********************************************************************/

#ifndef _CFG_SPEED_ALM_OUTPUT_DLG_H_
#define _CFG_SPEED_ALM_OUTPUT_DLG_H_

#include "ListCtrl.h"
#include "CfgDlg.h"
#include "CfgTriggerDlg.h"

using namespace GUI;

class CCfgSpeedAlmOutputDlg : public CCfgDlg
{
public:
	CCfgSpeedAlmOutputDlg();
	~CCfgSpeedAlmOutputDlg();

	//unsigned long PreCheckValue();

protected:
	void OnInitial();
	void OnDestroy();
	//void SetTip();
	//void OnChangeFocus(unsigned long oldID, unsigned long newID);

protected: 
	bool ShowData(NCFG_ITEM_ID cfgID, const BYTE* pData, int len, bool bRefresh);
	bool SetData(NCFG_ITEM_ID cfgID, BYTE*& pData, int& len)  const;


	DECLARE_MSG_MAP()
	unsigned long OnClickList(unsigned long ctrlID, unsigned long keyID, unsigned long xPos, unsigned long yPos);

protected:
	GUI::CListCtrl m_listCtrl;

	enum COL_INDEX
	{
		COL_CHANNEL,
		COL_HOLD_TIME,
		COL_TRIGGER,
	};

	ALARM_TRIGGER_INFO* m_pTriggerInfo;

};




#endif

