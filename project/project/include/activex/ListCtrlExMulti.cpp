...
template <class T>
GUI::CListItemExMulti<T>::CListItemExMulti()
{
}
...
template <class T>
bool GUI::CListItemExMulti<T>::Create(unsigned long ctrlID, unsigned long x, unsigned long y, unsigned long cx, unsigned long cy, CWnd* pParent, int xFocus, int yFocus)
{
	return GUI::CWnd::Create(ctrlID, x, y, cx, cy, pParent, WND_CHILD | WND_VISIBLE | WND_NO_IMAGE_BK, xFocus, yFocus);
}

template <class T>
void GUI::CListItemExMulti<T>::OnPaint(PAINT_STATUS status)
{
	//需要刷新本控件时候,通知父窗口去刷新
	GUI::CListCtrlExMulti<T>* pListCtrlEx3 = dynamic_cast<GUI::CListCtrlExMulti<T>*>(GetParent());
	assert(pListCtrlEx3);
	pListCtrlEx3->OnDrawItem(static_cast<int>(GetData()), status);

	//通知父窗口处理获取焦点事件
	if(PAINT_STATUS_FOCUS == status)
	{
		pListCtrlEx3->OnFocusItem(static_cast<int>(GetData()));
	}
}
...
template <class T>
GUI::CListCtrlExMulti<T>::CListCtrlExMulti() : m_bSelect(false)
, m_currLine(-1)
, m_pageNum(0)
, m_curPage(0)
, m_lineNum(0)
...
template <class T>
GUI::CListCtrlExMulti<T>::~CListCtrlExMulti()
{

}

template <class T>
bool GUI::CListCtrlExMulti<T>::Create(unsigned long ctrlID, 
							unsigned long x, 
							unsigned long y, 
							unsigned long cx, 
							unsigned long cy, 
...
template <class T>
void GUI::CListCtrlExMulti<T>::PutData(T * pData, unsigned long num, bool bRefresh/* = true*/)
{
	//可以为空，为空则刷新
	//assert (NULL != pData);
	//assert (num > 0);
...
template <class T>
void GUI::CListCtrlExMulti<T>::AddItem(const T& item, bool bRefresh)
{
...
template <class T>
void GUI::CListCtrlExMulti<T>::DelItem(long item, bool bRefresh)
{
...
template <class T>
T* GUI::CListCtrlExMulti<T>::GetSelectData(unsigned long & count) const
{
...
template <class T>
void GUI::CListCtrlExMulti<T>::OnInitial()
{
	GUI::CWnd::OnInitial();
	...
	m_pListItemVir = new GUI::CListItemExMulti<T>[m_lineNum];
	...
	m_bEnableMultiSel = true;
}

template <class T>
void GUI::CListCtrlExMulti<T>::OnDestroy()
{
	GUI::CWnd::OnDestroy();
...
template <class T>
void GUI::CListCtrlExMulti<T>::OnPaint(PAINT_STATUS status)
{
	//清除背景
...
template <class T>
int GUI::CListCtrlExMulti<T>::AddColumn(const char* pStr, int width)
{
	assert(NULL != pStr);
...
template <class T>
void GUI::CListCtrlExMulti<T>::RefreshCheckStatus(bool bRefresh)
{
	assert(m_curPage > 0);
...
template <class T>
bool GUI::CListCtrlExMulti<T>::IsItemSelect(int item) const
{
	for (SELECT_LIST::const_iterator iter = m_selectList.begin(); iter != m_selectList.end(); ++iter)
	{
		if (*iter == item)
...
template <class T>
BEGIN_MSG_MAP(GUI::CListCtrlExMulti<T>, GUI::CWnd)
...
//////////////////////////////////////////////////////////////////////////
template <class T>
unsigned long GUI::CListCtrlExMulti<T>::OnFirstPage()
{
...
template <class T>
unsigned long GUI::CListCtrlExMulti<T>::OnPrevPage()
{
	if ((m_itemNum > m_lineNum) && (0 != m_currItem))
	{
		unsigned long i = 0;
...
template <class T>
unsigned long GUI::CListCtrlExMulti<T>::OnNextPage()
{
...
template <class T>
unsigned long GUI::CListCtrlExMulti<T>::OnEndPage()
{
	if ((m_itemNum > m_lineNum) && ((m_currItem+m_lineNum) < m_itemNum))
	{
...
template <class T>
unsigned long GUI::CListCtrlExMulti<T>::OnSelAll()
{
	if (0 < m_itemNum)
	{
...
template <class T>
unsigned long GUI::CListCtrlExMulti<T>::OnClickCheck(unsigned long ctrlID)
{
...
template <class T>
unsigned long GUI::CListCtrlExMulti<T>::OnClickList(unsigned long ctrlID, unsigned long keyID, unsigned long xPos, unsigned long yPos)
{
	...
	for (int i = 0; i < m_lineNum; i++ )
	{
		if(m_pListItemVir[i].GetID() == ctrlID)
		{
			...
			int currItem = i + (m_curPage - 1) * m_lineNum;
			if(m_pListItemVir[i].IsEnable())
			{
				if(m_multiSelectList.size() == 0)
				{
					m_multiSelectList.push_back(currItem);
					break;
				}
				else
				{
					if(m_multiSelectList[m_multiSelectList.size()-1] < currItem)
					{
						m_multiSelectList.push_back(currItem);
						break;
					}
					...
					{
									for(int i=0;  i < m_multiSelectList.size(); i++)
									{
										if(m_multiSelectList[i] == currItem)
										{
											m_multiSelectList.erase(m_multiSelectList.begin() + i);
											break;
										}
										else if(m_multiSelectList[i] > currItem)
										{
											m_multiSelectList.insert(m_multiSelectList.begin()+i, currItem);
											break;
										}
									}
									break;
		...
		if( (m_width <= xPos) || (m_height <= yPos))
		{
			...
			if(hitIndex >= 0)
			{
				...
				if ((item >= 0) && (item < m_itemNum))
				{
					...
					{
						DrawItem(m_singleSelItem, ITEM_BG | ITEM_RC | ITEM_SEL_BG, true);
	...
	if (m_bEnableMultiSel)//设置可多选
	{
		if( (m_width <= xPos) || (m_height <= yPos))
		{
			...
			if(hitIndex >= 0)
			{
				...
				if ((item >= 0) && (item < m_itemNum))
				{
					for(int j=0; j<m_multiSelectList.size(); j++)
					{
						if( 0 == m_multiSelectList.size() )
						{
							DrawItem(item, ITEM_BG | ITEM_RC , true);
							return KEY_VIRTUAL_MSG_CONTINUE;
						}
						if(item == m_multiSelectList[j])
						{
							DrawItem(item, ITEM_BG | ITEM_RC | ITEM_FOCUS | ITEM_SEL_BG, true);
							break;
						}
						if(j == m_multiSelectList.size() - 1)
						{
							DrawItem(item, ITEM_BG | ITEM_RC , true);
						}
					}
...
template <class T>
unsigned long GUI::CListCtrlExMulti<T>::OnDBClickList(unsigned long ctrlID, unsigned long keyID, unsigned long xPos, unsigned long yPos)
{
	/*
		双击、屏蔽无效数据区域的消息
	*/
...
template <class T>
long GUI::CListCtrlExMulti<T>::LineIndex(unsigned long xPos, unsigned long yPos)
{
...
template <class T>
void GUI::CListCtrlExMulti<T>::ShowData(PAINT_STATUS status)
{
	if (m_itemNum > 0)
	{
...
template<class T>
void GUI::CListCtrlExMulti<T>::SetSingleSelect(long item, bool bRefresh)
{
...
template<class T>
long GUI::CListCtrlExMulti<T>::ForEach(long param, bool bRefresh /* = false */)
{
	for (int i = 0; i < m_itemNum; ++i)
	{
...
template<class T>
const T* GUI::CListCtrlExMulti<T>::GetItem(int item) const
{
...
template<class T>
T* GUI::CListCtrlExMulti<T>::GetItem(int item) 
{
	if ((NULL != m_pData) && (item >= 0) && (item < m_itemNum))
	{
		return &(m_pData[item]);
...
template<class T>
void GUI::CListCtrlExMulti<T>::CleanSelected( ) 
{
...
template<class T>
void GUI::CListCtrlExMulti<T>::SetSelAll( ) 
{
	if (!m_ckAll.IsCheck())
	{
...
template<class T>
void GUI::CListCtrlExMulti<T>::DrawItem(long item, unsigned long style, bool bRefresh)
{
	/*
...
template<class T>
void GUI::CListCtrlExMulti<T>::OnDrawItem(int index, PAINT_STATUS status)
{
	assert(index >= 0);
	...
	{	
		if(!m_multiSelectList.empty())
		{
			for (SELECT_LIST::const_iterator iter = m_multiSelectList.begin(); iter != m_multiSelectList.end(); ++iter)
			{
				if (*iter == item)
				{
					style |= ITEM_SEL_BG;
...
template<class T>
void GUI::CListCtrlExMulti<T>::OnFocusItem(int index)
{
	/*
