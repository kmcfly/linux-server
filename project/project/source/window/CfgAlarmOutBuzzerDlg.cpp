...
unsigned long CCfgAlarmOutBuzzerDlg::s_holdTime[] = {5, 10, 20, 30, 60, 120, -1};

CCfgAlarmOutBuzzerDlg::CCfgAlarmOutBuzzerDlg()
{
...
void CCfgAlarmOutBuzzerDlg::OnInitial()
{
	CCfgDlg::OnInitial();
	unsigned short x = 0;
	unsigned short y = 0;
...
bool CCfgAlarmOutBuzzerDlg::ShowData(NCFG_ITEM_ID cfgID, const BYTE* pData, int len, bool bRefresh)
{
	assert(NULL != pData);
	assert(len > 0);
...
bool CCfgAlarmOutBuzzerDlg::SetData(NCFG_ITEM_ID cfgID, BYTE*& pData, int& len) const
{
	assert(NULL != pData);
	assert(len > 0);
...
BEGIN_MSG_MAP(CCfgAlarmOutBuzzerDlg, CCfgDlg)
	ON_MSG_EX(m_listCtrl.GetID(), KEY_VIRTUAL_MSG_DEFAULT_TAG, OnClickList)
END_MSG_MAP()

void CCfgAlarmOutBuzzerDlg::GetItemList(CFGITEM_LIST& itemList) const
{
	if (!IsSupportAlarmOut())
	{
...
unsigned long CCfgAlarmOutBuzzerDlg::OnClickList(unsigned long ctrlID, unsigned long keyID, unsigned long xPos, unsigned long yPos )
{
