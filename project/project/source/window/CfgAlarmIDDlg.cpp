...
void CCfgAlarmIdDlg::OnInitial()
{
	...
	list.AddColumn("AlarmID", width * 16 / 32);	//name
...
bool CCfgAlarmIdDlg::ShowData(NCFG_ITEM_ID cfgID, const BYTE* pData, int len, bool bRefresh)
{
	assert(NULL != pData);
	assert(len > 0);
	GUI::CListCtrl& list = m_cfgLiveList;
	...
	case NCFG_ITEM_GB28181_ALARM_ID:
		{
			...
			const GB28181_ALARM_ID* pShortName = reinterpret_cast<const GB28181_ALARM_ID*>(pData);
			for(int i = 0; i< effectChnn; i++)
			{
				...
				pEdit->SetCaption(pShortName->alarm_id[i], bRefresh);
			}
		}
		return true;
		break;
...
bool CCfgAlarmIdDlg::SetData(NCFG_ITEM_ID cfgID, BYTE*& pData, int& len)  const
{
	assert(NULL != pData);
	assert(len > 0);
	...
	case NCFG_ITEM_GB28181_ALARM_ID:
		{
			int numChnn = maxChnn; //CCfgDlg::s_loginSuccessReply.productInfo.localVideoInputNum;
			assert(len == sizeof(GB28181_ALARM_ID));

			GB28181_ALARM_ID* pShortName = reinterpret_cast<GB28181_ALARM_ID*>(pData);
			for (int i = 0; i < effectChnn; i++)
			{
				...
				strcpy(pShortName->alarm_id[i], pEdit->GetCaption().c_str());
			}
		}
		return true;
		break;
...
BEGIN_MSG_MAP(CCfgAlarmIdDlg, GUI::CDialog)
	ON_MSG_EX(m_cfgLiveList.GetID(), KEY_VIRTUAL_MSG_DEFAULT_TAG, OnClickList)
END_MSG_MAP()

unsigned long CCfgAlarmIdDlg::OnCheck()
{
	return KEY_VIRTUAL_MSG_STOP;
}

unsigned long CCfgAlarmIdDlg::OnClickList(unsigned long ctrlID, unsigned long keyID, unsigned long xPos, unsigned long yPos)
{
...
unsigned long CCfgAlarmIdDlg::PreCheckValue()
{
	if(IsChange(NCFG_ITEM_GB28181_ALARM_ID)) 
	{
...
bool CCfgAlarmIdDlg::IsValidID(const string &strID)
{
	if (strID == "")
	{
		return true;
...
unsigned long CCfgAlarmIdDlg::CheckValue()
{
