...
** Author       : ZXX
** Date         : 2013-11-26
** Name         : WzdNatGetNetWorkSetUpDlg.h
** Version      : 1.0
** Description  : 向导管理对话框
** Modify Record:
***********************************************************************/
...
CWzdNatGetNetWorkSetUpDlg::CWzdNatGetNetWorkSetUpDlg()
{
	m_toprow = 0;
...
void CWzdNatGetNetWorkSetUpDlg::SetWzdInfo(NAT_WIZARD_INFO* pWzdInfo)
{
	m_pNetWzdInfo = pWzdInfo;
	
}

void CWzdNatGetNetWorkSetUpDlg::InitCtrlInfo()
{

}

void CWzdNatGetNetWorkSetUpDlg::ShowWzdData(bool bRefresh)
{
	...
	if(bret)
	{
		...
		str.append(m_pStrTable->String(E_STR_ID_NAT_WZD_NETWORK_0e));
		str.append("\\n");
		// str.append("Your DVR is connected to your network.");
		str.append(m_pStrTable->String(E_STR_ID_NAT_WZD_NETWORK_0f));
		str.append("\\n");
		// str.append("The local IP address for this DVR is ");
		str.append(m_pStrTable->String(E_STR_ID_NAT_WZD_NETWORK_10));
		str.append("http://");
		str.append(ipArr);
		str.append(":");
		char port[5]={0};
		...
		str.append(port);
		str.append("\\n");
		// str.append("If you will be connecting to your DVR away from your local network (over the Internet), please click Next to continue with the Peer-to-Peer setup. ");
		str.append(m_pStrTable->String(E_STR_ID_NAT_WZD_NETWORK_12));
		str.append("\\n");
		str.append(" \\n");
		str.append(" \\n");
		...
		str.append(m_pStrTable->String(E_STR_ID_NAT_WZD_NETWORK_13));
...
void CWzdNatGetNetWorkSetUpDlg::GetWzdData(bool bNext)
{
	
}

bool CWzdNatGetNetWorkSetUpDlg::GetNatEnaleFlag()
{
	return m_bEnableNatFlag;
}
void CWzdNatGetNetWorkSetUpDlg::OnInitial()
{
	...
	m_rdNat.Create(GetRandWndID(),x,y,2*m_length[BTN_WIDTH],m_length[RADIO_HEIGHT],this,0,2,&m_rdGroup);
	...
	// m_rdNat.SetCaption("Local and Internet Connection",false,false);
	m_rdNat.SetCaption(m_pStrTable->String(E_STR_ID_NAT_WZD_NETWORK_0c), false, false);
	m_rdNat.SetData(NET_NAT);
	// m_rdLocal.SetCaption("Local Connection Only",false,false);
	m_rdLocal.SetCaption(m_pStrTable->String(E_STR_ID_NAT_WZD_NETWORK_0d), false, false);
...
void CWzdNatGetNetWorkSetUpDlg::OnDestroy()
{
	
}

bool CWzdNatGetNetWorkSetUpDlg::GetIp()
{
...
void CWzdNatGetNetWorkSetUpDlg::GetStaticIp(unsigned long ip,char *pIP)
{
	char str[32]= {0};
...
BEGIN_MSG_MAP(CWzdNatGetNetWorkSetUpDlg, CWzdNatPageDlg)
...
unsigned long  CWzdNatGetNetWorkSetUpDlg::OnCheck()
{
...
unsigned long CWzdNatGetNetWorkSetUpDlg::OnScrollchange(unsigned long ctrlID, unsigned long keyID, unsigned long xPos, unsigned long yPos)
{
...
unsigned long CWzdNatGetNetWorkSetUpDlg::OnClickRadioCtrl()
{
