...
CWzdNewNatQrCodeDlg::CWzdNewNatQrCodeDlg()
{
	m_pMaix = NULL;
	m_drawStartX = 0;
	m_drawStartY = 0;
...
void CWzdNewNatQrCodeDlg::SetWzdInfo(NAT_WIZARD_INFO* pWzdInfo)
{
	m_pWzdInfo = pWzdInfo;
}

void CWzdNewNatQrCodeDlg::ShowWzdData(bool bRefresh)
{
	...
	/*		{*/
	// strTip.append("Mobile Devices Setup for Android and Apple:");
	strTip.append(m_pStrTable->String(E_STR_ID_US02_NEW_WZD_QRCODE_00));
	strTip.append("\\n");
	// strTip.append("Phone: Download QT View App from Play Store (Android) or App Store (Apple)");
	strTip.append(m_pStrTable->String(E_STR_ID_US02_NEW_WZD_QRCODE_01));
	strTip.append("\\n");
	// strTip.append("Tablet: Download QT View HD App from Play Store (Android) or App Store (Apple)");
	strTip.append(m_pStrTable->String(E_STR_ID_US02_NEW_WZD_QRCODE_02));
	strTip.append("\\n");
	// strTip.append("1.Launch QT View app and click on the Bar Code Icon at the bottom");
	strTip.append(m_pStrTable->String(E_STR_ID_US02_NEW_WZD_QRCODE_03));
	strTip.append("\\n");
	// strTip.append("2.Scan this QR Code");
	strTip.append(m_pStrTable->String(E_STR_ID_US02_NEW_WZD_QRCODE_04));
	strTip.append("\\n");
	// strTip.append("3.Enter the User name: admin")
	strTip.append(m_pStrTable->String(E_STR_ID_US02_NEW_WZD_QRCODE_05));
	strTip.append("\\n");
	// strTip.append("4.Enter the DVR Password");
	strTip.append(m_pStrTable->String(E_STR_ID_US02_NEW_WZD_QRCODE_06));
	...
	y = y+cy+cy/2 - 2*(m_length[RADIO_HEIGHT]+2);

	m_rdDevPc.Create(GetRandWndID(),x + m_length[BTN_WIDTH],y,2*m_length[BTN_WIDTH],m_length[RADIO_HEIGHT],this,0,3);
	m_rdDevPc.SetCaption(m_pStrTable->String(E_STR_ID_US02_NEW_WZD_SELECT_DEV_01) /*"Computer Setup"*/,false,false);
	m_rdDevPc.SetData(NEXT_PC);
...
void CWzdNewNatQrCodeDlg::GetWzdData(bool bNext)
{

}

void CWzdNewNatQrCodeDlg::OnLeavePage()
{

}


void CWzdNewNatQrCodeDlg::SetMaix(unsigned char * pMaix,unsigned long w,unsigned long h)
{
	assert(w > 0);
	assert(h > 0);
	assert(pMaix != NULL);
...
void CWzdNewNatQrCodeDlg::OnInitial()
{
	m_bSelect = false;
...
void CWzdNewNatQrCodeDlg::SetDevIndex(int index)
{
	m_devIndex = index;
	m_toprow = 0;
}

void CWzdNewNatQrCodeDlg::InitCtrlInfo()
{

}

void CWzdNewNatQrCodeDlg::OnPaint(PAINT_STATUS status)
{
...
void CWzdNewNatQrCodeDlg::OnDestroy()
{
	if(NULL != m_pMaix)
	{
		delete []m_pMaix;
...
BEGIN_MSG_MAP(CWzdNewNatQrCodeDlg, CWzdNatPageDlg)
...
unsigned long  CWzdNewNatQrCodeDlg::OnCheck()
{
...
unsigned long CWzdNewNatQrCodeDlg::OnScrollchange(unsigned long ctrlID, unsigned long keyID, unsigned long xPos, unsigned long yPos)
{
	switch(m_devIndex)
	{
...
unsigned long CWzdNewNatQrCodeDlg::OnClickRadioCtrl()
{

	if(m_rdDevPc.IsCheck())
	{
 		m_bSelect = true;
