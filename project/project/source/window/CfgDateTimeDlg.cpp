...
CCfgDateTimeDlg::CCfgDateTimeDlg() : m_currTime(0), m_bStopRefreshTime(false)
{

}

CCfgDateTimeDlg::~CCfgDateTimeDlg()
{

}

void CCfgDateTimeDlg::OnInitial()
{
	CCfgDlg::OnInitial();
	//////////////////////////////////////////////////////////////////////////
	...
	for (int i = 0; i < count; ++i)
	{
			...
			{
				pCombo->AddItem(m_pStrTable->String(id), i);
			}
		#else
...
unsigned long CCfgDateTimeDlg::RecoverChange()
{
...
void CCfgDateTimeDlg::SetModifyTime(bool modify, bool bRefresh)
{
...
unsigned long CCfgDateTimeDlg::OnPreCommand(unsigned long ctrlID, unsigned long keyID, unsigned long xPos, unsigned long yPos)
{
	if( m_bUpdateTimeNow )
	{
		//在NTP即时同步的时候屏蔽其他消息响应
...
void CCfgDateTimeDlg::OnChangeFocus(unsigned long oldID, unsigned long newID)
{
...
//void CCfgDateTimeDlg::CheckUI(bool bRefresh)
//{
//	GUI::CListCtrl& list = m_cfgDateTimeList;
//	
//	//dst
...
bool CCfgDateTimeDlg::ShowData(NCFG_ITEM_ID cfgID, const BYTE* pData, int len, bool bRefresh)
{
	assert(NULL != pData);
	assert(len > 0);
...
bool CCfgDateTimeDlg::SetData(NCFG_ITEM_ID cfgID, BYTE*& pData, int& len)  const
{
	assert(NULL != pData);
	assert(len > 0);
...
BEGIN_MSG_MAP(CCfgDateTimeDlg, CCfgDlg)
...
unsigned long CCfgDateTimeDlg::OnClickList(unsigned long ctrlID, unsigned long keyID, unsigned long xPos, unsigned long yPos)
{
...
void CCfgDateTimeDlg::SetEnableUI( LIST_ROW rowNum, bool brefalsh )
{
	switch( rowNum )
	{
#if defined(__DVR_BASIC__) && !defined(__TW01_RILI__)
...
unsigned long CCfgDateTimeDlg::OnMouseWheel()
{
