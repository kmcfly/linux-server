...
static int soap_try_connect_command(struct soap*, int http_command, const char *endpoint, const char *action, const char *pEther);
#ifndef WITH_NOIDREF
...
#endif
static SOAP_SOCKET tcp_connect(struct soap*, const char *endpoint, const char *host, int port, const char *pEther);
...
#ifndef WITH_NOIO
#ifndef PALM_1
static int fsend(struct soap *aSoap, const char *s, size_t n)
{ 
	register int nwritten, err;
#if 0//defined(__cplusplus) && !defined(WITH_LEAN) && !defined(WITH_COMPAT)
  if (aSoap->os)
  { 
	aSoap->os->write(s, (std::streamsize)n);
    if (aSoap->os->good())
	{
		return SOAP_OK;
	}
  ...
  while (n)
  { 
	 if (soap_valid_socket(aSoap->socket))
    {
      if (aSoap->send_timeout)
      { 
		 for (;;)
        { 
		   register int r;
...
#endif
            r = tcp_select(aSoap, aSoap->socket, SOAP_TCP_SELECT_SND | SOAP_TCP_SELECT_ERR, aSoap->send_timeout);
          if (r > 0)
		  {
            break;
		  }
...
#endif
#ifndef WITH_LEAN
      if ((aSoap->omode & SOAP_IO_UDP))
      { 
		 if (aSoap->peerlen)
		 {
			 nwritten = sendto(aSoap->socket, (char*)s, (SOAP_WINSOCKINT)n, aSoap->socket_flags, (struct sockaddr*)&aSoap->peer, (SOAP_WINSOCKINT)aSoap->peerlen);
		 }
		 else
		 {
			 nwritten = send(aSoap->socket, s, (SOAP_WINSOCKINT)n, aSoap->socket_flags);
		 }
        ...
        { 
		   int udp_repeat;
          int udp_delay;
          if ((aSoap->connect_flags & SO_BROADCAST))
		  {
          ...
          do
          { 
				  tcp_select(aSoap, aSoap->socket, SOAP_TCP_SELECT_ERR, -1000 * udp_delay);
				 if (aSoap->peerlen)
				{
					nwritten = sendto(aSoap->socket, (char*)s, (SOAP_WINSOCKINT)n, aSoap->socket_flags, (struct sockaddr*)&aSoap->peer, (SOAP_WINSOCKINT)aSoap->peerlen);
				}
        ...
        nwritten = send(aSoap->socket, s, (int)n, aSoap->socket_flags);
#else
      ...
      {
        register int r = 0;
        err = soap_socket_errno(aSoap->socket);
#ifdef WITH_OPENSSL
        ...
        if (err == SOAP_EWOULDBLOCK || err == SOAP_EAGAIN)
        {
          ...
          if (aSoap->ssl && r == SSL_ERROR_WANT_READ)
		  {
			  r = tcp_select(aSoap, aSoap->socket, SOAP_TCP_SELECT_RCV | SOAP_TCP_SELECT_ERR, aSoap->send_timeout ? aSoap->send_timeout : -10000);
		  }
		  else
#elif defined(WITH_GNUTLS)
          if (aSoap->session && !gnutls_record_get_direction(aSoap->session))
		  {
			  r = tcp_select(aSoap, aSoap->socket, SOAP_TCP_SELECT_RCV | SOAP_TCP_SELECT_ERR, aSoap->send_timeout ? aSoap->send_timeout : -10000);
		  }
		  else
#endif
          r = tcp_select(aSoap, aSoap->socket, SOAP_TCP_SELECT_SND | SOAP_TCP_SELECT_ERR, aSoap->send_timeout ? aSoap->send_timeout : -10000);
         if (!r && aSoap->send_timeout)
		  {
			  return SOAP_EOF;
		  }
          if (r < 0 && aSoap->errnum != SOAP_EINTR)
		  {
			  return SOAP_EOF;
		  }
        }
...
#ifdef WMW_RPM_IO
      if (aSoap->rpmreqid)
        nwritten = (httpBlockPut(aSoap->rpmreqid, (char*)s, n) == 0) ? n : -1;
      else
#endif
        nwritten = fwrite(s, sizeof(char), n, fdopen(aSoap->sendfd, "w"));
#else

	#ifdef WIN32
			nwritten = _write(aSoap->sendfd, s, (unsigned int)n);
	#else				  

			//printf("before write!\n"); //modify by luomm in 2014.03.11
			//nwritten = write(aSoap->sendfd, s, (unsigned int)n);
			nwritten = n;
	#endif
...
#else
	  if(soap->recvfd > 2) //modify by luomm in 2014.03.11
	  {
...
#ifndef PALM_1
static SOAP_SOCKET
tcp_connect(struct soap *soap, const char *endpoint, const char *host, int port, const char *pEther)
{
#ifdef WITH_IPV6
...
//设置绑定到物理设备
  ...
  if(NULL != pEther)
  {
	  setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, pEther, strlen(pEther) + 1);
  }
#endif
...
#ifndef PALM_1
static int
tcp_disconnect(struct soap *aSoap)
{
#ifdef WITH_OPENSSL
  if (aSoap->ssl)
  { 
    ...
    if (aSoap->session)
    { 
	  SSL_SESSION_free(aSoap->session);
      aSoap->session = NULL;
    }
    if (*aSoap->host)
    { 
	  aSoap->session = SSL_get1_session(aSoap->ssl);
      if (aSoap->session)
      { 
		strcpy(aSoap->session_host, aSoap->host);
        aSoap->session_port = aSoap->port;
      }
    }
    r = SSL_shutdown(aSoap->ssl);
    /* SSL shutdown does not work when reads are pending, non-blocking */
    if (r == 0)
    { 
	  while (SSL_want_read(aSoap->ssl))
      { 
		 if (SSL_read(aSoap->ssl, NULL, 0)
         || soap_socket_errno(aSoap->socket) != SOAP_EAGAIN)
        { 
			r = SSL_shutdown(aSoap->ssl);
			break;
        }
    ...
    { 
	  if (soap_valid_socket(aSoap->socket))
      { 
		 if (!aSoap->fshutdownsocket(aSoap, aSoap->socket, SOAP_SHUT_WR))
        {
          ...
          timeout)
          */
          r = tcp_select(aSoap, aSoap->socket, SOAP_TCP_SELECT_RCV | SOAP_TCP_SELECT_ERR, 5);
          if (r <= 0 && soap->errnum != SOAP_EINTR)
          { 
			aSoap->errnum = 0;
            //DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Connection lost...\n"));
            aSoap->fclosesocket(aSoap, aSoap->socket);
            aSoap->socket = SOAP_INVALID_SOCKET;
            ERR_remove_state(0);
            SSL_free(aSoap->ssl);
            aSoap->ssl = NULL;
            return SOAP_OK;
    ...
    if (r != 1)
    {
      ...
      if (s)
      { 
		//DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Shutdown failed: %d\n", SSL_get_error(aSoap->ssl, r)));
        if (soap_valid_socket(aSoap->socket) && !(aSoap->omode & SOAP_IO_UDP))
        { 
		  aSoap->fclosesocket(aSoap, aSoap->socket);
          aSoap->socket = SOAP_INVALID_SOCKET;
        }
  ...
  if (aSoap->session)
  { 
	  gnutls_bye(aSoap->session, GNUTLS_SHUT_RDWR);
	  gnutls_deinit(aSoap->session);
	  aSoap->session = NULL;
  }
#endif
  if (soap_valid_socket(aSoap->socket) && !(aSoap->omode & SOAP_IO_UDP))
  { 
	  aSoap->fshutdownsocket(aSoap, aSoap->socket, SOAP_SHUT_RDWR);
      aSoap->fclosesocket(aSoap, aSoap->socket);
      aSoap->socket = SOAP_INVALID_SOCKET;
  }
  return SOAP_OK;
...
/******************************************************************************/
#ifndef PALM_1
SOAP_FMAC1 void SOAP_FMAC2 soap_done(struct soap *aSoap)
{
#ifdef SOAP_DEBUG
  int i;
#endif
  ...
  if (soap_check_state(aSoap))
  {
	  return;
  }
  //DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Done with context\n"));
  soap_free_temp(aSoap);
  while (aSoap->clist)
  { 
	  struct soap_clist *p = aSoap->clist->next;
    SOAP_FREE(aSoap, aSoap->clist);
    aSoap->clist = p;
  }
  if (aSoap->state == SOAP_INIT)
  {
	  aSoap->omode &= ~SOAP_IO_UDP; /* to force close the socket */
  }
  aSoap->keep_alive = 0; /* to force close the socket */
  soap_closesock(aSoap);
#ifdef WITH_COOKIES
  soap_free_cookies(soap);
#endif
  while (aSoap->plugins)
  { 
	 register struct soap_plugin *p = aSoap->plugins->next;
    //DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Removing plugin '%s'\n", aSoap->plugins->id));
    if (aSoap->plugins->fcopy || aSoap->state == SOAP_INIT)
	{
		aSoap->plugins->fdelete(aSoap, aSoap->plugins);
	}
    SOAP_FREE(aSoap, aSoap->plugins);
    aSoap->plugins = p;
  }
  aSoap->fplugin = fplugin;
  aSoap->fmalloc = NULL;
#ifndef WITH_NOHTTP
  aSoap->fpost = http_post;
  aSoap->fput = http_put;
  aSoap->fget = http_get;
  aSoap->fdel = http_405;
  aSoap->fopt = http_405;
  aSoap->fhead = http_405;
  aSoap->fform = NULL;
  aSoap->fposthdr = http_post_header;
  aSoap->fresponse = http_response;
  aSoap->fparse = http_parse;
  aSoap->fparsehdr = http_parse_header;
#endif
  aSoap->fheader = NULL;
#ifndef WITH_NOIO
#ifndef WITH_IPV6
  aSoap->fresolve = tcp_gethost;
#else
  aSoap->fresolve = NULL;
#endif
  aSoap->faccept = tcp_accept;
  aSoap->fopen = tcp_connect;
  aSoap->fclose = tcp_disconnect;
  aSoap->fclosesocket = tcp_closesocket;
  aSoap->fshutdownsocket = tcp_shutdownsocket;
  aSoap->fsend = fsend;
  aSoap->frecv = frecv;
  aSoap->fpoll = soap_poll;
#else
  aSoap->fopen = NULL;
  aSoap->fclose = NULL;
  aSoap->fpoll = NULL;
#endif
#ifndef WITH_LEANER
  aSoap->feltbegin = NULL;
  aSoap->feltendin = NULL;
  aSoap->feltbegout = NULL;
  aSoap->feltendout = NULL;
  aSoap->fprepareinitsend = NULL;
  aSoap->fprepareinitrecv = NULL;
  aSoap->fpreparesend = NULL;
  aSoap->fpreparerecv = NULL;
  aSoap->fpreparefinalsend = NULL;
  aSoap->fpreparefinalrecv = NULL;
  aSoap->ffiltersend = NULL;
  aSoap->ffilterrecv = NULL;
#endif
  aSoap->fseterror = NULL;
  aSoap->fignore = NULL;
  aSoap->fserveloop = NULL;
#ifdef WITH_OPENSSL
  if (aSoap->session)
  { 
	  SSL_SESSION_free(aSoap->session);
  ...
  if (aSoap->state == SOAP_INIT)
  { 
	  if (soap_valid_socket(aSoap->master))
		{ 
			aSoap->fclosesocket(aSoap, aSoap->master);
		   aSoap->master = SOAP_INVALID_SOCKET;
		}
  }
#ifdef WITH_OPENSSL
  if (aSoap->ssl)
  ...
  { 
	  if (aSoap->xcred)
    { 
		gnutls_certificate_free_credentials(aSoap->xcred);
        aSoap->xcred = NULL;
    }
    if (aSoap->acred)
    { 
		gnutls_anon_free_client_credentials(aSoap->acred);
      aSoap->acred = NULL;
    }

    if (aSoap->cache)
    { 
		gnutls_priority_deinit(aSoap->cache);
        aSoap->cache = NULL;
    }
    if (aSoap->dh_params)
    { 
		gnutls_dh_params_deinit(aSoap->dh_params);
        aSoap->dh_params = NULL;
    }
    if (aSoap->rsa_params)
    { 
		gnutls_rsa_params_deinit(aSoap->rsa_params);
      aSoap->rsa_params = NULL;
    }
  }
  if (aSoap->session)
  { 
  ...
  if (aSoap->d_stream)
  { 
	  SOAP_FREE(aSoap, (void*)aSoap->d_stream);
    aSoap->d_stream = NULL;
  }
  if (aSoap->z_buf)
  { 
	  SOAP_FREE(aSoap, (void*)aSoap->z_buf);
    aSoap->z_buf = NULL;
  }
#endif
#ifdef SOAP_DEBUG
  ...
  { 
	  if (aSoap->logfile[i])
    { SOAP_FREE(aSoap, (void*)aSoap->logfile[i]);
      aSoap->logfile[i] = NULL;
    }
    soap_close_logfile(aSoap, i);
  }
  aSoap->state = SOAP_NONE;
#endif
#ifdef SOAP_MEM_DEBUG
  soap_free_mht(aSoap);
#endif
...
#ifdef WITH_COOKIES
/******************************************************************************/
SOAP_FMAC1 size_t SOAP_FMAC2 soap_encode_cookie(const char *s, char *t, size_t len)
{ register int c;
...
/******************************************************************************/
SOAP_FMAC1 struct soap_cookie* SOAP_FMAC2 soap_set_cookie(struct soap *soap, const char *name, const char *value, const char *domain, const char *path)
...
{ 
  struct soap_plist *pp;
  if (p == NULL || (soap->mode & SOAP_XML_TREE))
  {
    return 1;
  }
  if (soap_pointer_lookup(soap, p, t, &pp))
...
int
SOAP_FMAC2
soap_connect(struct soap *soap, const char *endpoint, const char *action, const char *pEther)
{ 
	return soap_connect_command(soap, SOAP_POST, endpoint, action, pEther);
}
#endif
...
int
SOAP_FMAC2
soap_connect_command(struct soap *soap, int http_command, const char *endpoints, const char *action, const char *pEther)
{ 
	char *endpoint;
  const char *s;
  ...
  { endpoint = (char*)SOAP_MALLOC(soap, strlen(endpoints) + 1);
    ...
    { strncpy(endpoint, endpoints, s - endpoints);
      ...
      if (soap_try_connect_command(soap, http_command, endpoint, action, pEther) != SOAP_TCP_ERROR)
        break;
      if (!*s)
        break;
      soap->error = SOAP_OK;
    ...
    soap_try_connect_command(soap, http_command, endpoints, action, pEther);
  return soap->error;
}
#endif
...
#ifndef PALM_1
static int
soap_try_connect_command(struct soap *soap, int http_command, const char *endpoint, const char *action, const char *pEther)
...
#endif
      soap->socket = soap->fopen(soap, endpoint, soap->host, soap->port, pEther);
      if (soap->error)
        return soap->error;
